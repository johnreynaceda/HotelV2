...
class Dashboard extends Component
{
    ...
    public function mount()
    {
        ...
        $this->total_check_out = CheckinDetail::where('is_check_out', 1)
            ->whereDate('check_out_at', Carbon::today())
            ->count();
        $this->total_available_rooms = Room::where('branch_id', auth()->user()->branch_id)
            ->where('status', 'Available')
            ->whereDate('updated_at', Carbon::today())
            ->count();
        $this->total_cleaning_rooms = Room::where('branch_id', auth()->user()->branch_id)
