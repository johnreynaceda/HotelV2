<div>
  ...
  <div class="flex items-end justify-between">
    <div>
      ...
      <h1 class="text-3xl uppercase font-extrabold text-gray-600">Select rate </h1>
    </div>
    <div>
      @if ($steps == 1)
      ...
      @foreach ($rates as $rate)
        <button wire:key="{{ $rate->id }}rate" wire:click="selectRate({{ $rate->id }})" type="button">
          <div class="border-2 border-blue-500 bg-gray-50 h-40 relative overflow-hidden grid place-content-center rounded-2xl {{ $rate_id == $rate->id ? 'border-green-500' : 'border-blue-500' }}">
            <svg
              class="lg:h-40 sm:max-h-36 absolute top-0 -right-16 {{ $rate_id == $rate->id ? 'text-green-600 opacity-40' : 'text-gray-500 opacity-10' }}"
            ...
            <h1 class="lg:text-[2rem] sm:text-md font-extrabold uppercase text-gray-700"> {{ $rate->stayingHour->number }} HOURS
            </h1>
