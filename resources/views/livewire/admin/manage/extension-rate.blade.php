          ...
          <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
            <table class="min-w-full divide-y divide-gray-300">
              ...
              <tbody class="divide-y divide-gray-200 bg-white">
                @forelse ($extensionRates as $rates)
                  <tr>
                    <td class="whitespace-nowrap py-3 pl-4 pr-3 text-sm   text-gray-700 sm:pl-6">
                      {{ $rates->hour }} HOURS</td>
                    <td class="whitespace-nowrap py-3 pl-4 pr-3 text-sm   text-gray-700 sm:pl-6">
                      &#8369;{{ number_format($rates->amount, 2) }}</td>

                    <td class="relative whitespace-nowrap py-3 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                      <div class="flex space-x-2 justify-end">
                        <x-button icon="pencil-alt" wire:click="editExtension({{ $rates->id }})"
                          spinner="editExtension({{ $rates->id }})" label="Edit" xs />
                        <x-button wire:click="deleteExtension({{ $rates->id }})"
                          spinner="deleteExtension({{ $rates->id }})" icon="trash" label="Delete" xs />
                      </div>
                    </td>
                  </tr>
                @empty
  ...
  <x-modal wire:model.defer="add_modal" max-width="lg">
    <x-card title="Add New">
      <div class="flex flex-col space-y-3">
        <x-input wire:model.defer="hour" label="Hours" placeholder="" />
        <x-input wire:model.defer="amount" label="Amount" placeholder="" />
      </div>
      <x-slot name="footer">
        <div class="flex justify-end gap-x-2">
          <x-button flat label="Cancel" x-on:click="close" />
          <x-button wire:click="saveExtension" spinner="saveExtension" positive label="Save" />
        </div>
      </x-slot>
    </x-card>
  </x-modal>
  <x-modal wire:model.defer="edit_modal" max-width="lg">
    <x-card title="Update Extension">
      <div class="flex flex-col space-y-3">
        <x-input wire:model.defer="hour" label="Hours" placeholder="" />
        <x-input wire:model.defer="amount" label="Amount" placeholder="" />
      </div>
      <x-slot name="footer">
        <div class="flex justify-end gap-x-2">
          <x-button flat label="Cancel" x-on:click="close" />
          <x-button wire:click="updateExtension" spinner="updateExtension" positive label="Update" />
        </div>
      </x-slot>
    </x-card>
  </x-modal> --}}
  ...
  <x-modal wire:model.defer="add_modal" align="center" max-width="lg">
    <x-card>
      <div class="header flex space-x-2 items-center">
        ...
        <h1 class="text-lg font-semibold uppercase text-gray-600 ">Add New Extension Rate</h1>
      </div>
      <div class="mt-5 px-2 flex flex-col space-y-3">
        <x-input label="Hour" wire:model.defer="hour" />
      ...
      <x-slot name="footer">
        <div class="flex justify-end gap-x-4">
          ...
          <x-button positive right-icon="save-as" wire:click="saveExtension" spinner="saveExtension" label="Save" />
        </div>
      </x-slot>
    </x-card>
  </x-modal>
