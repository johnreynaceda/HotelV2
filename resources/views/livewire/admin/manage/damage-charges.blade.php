<div>
    ...
    <x-card title="Add New">
      ...
      <x-slot name="footer">
        <div class="flex justify-end gap-x-2">
          ...
          <x-button wire:click="saveCharges" spinner="saveCharges" positive label="Save" />
        </div>
      </x-slot>
    </x-card>
  </x-modal>
  ...
  <x-modal wire:model.defer="edit_modal" max-width="lg">
    <x-card title="Update Damage Charges">
      <div class="flex flex-col space-y-3">
        <x-input wire:model.defer="name" label="Name" placeholder="" />
        <x-input wire:model.defer="amount" label="Amount" placeholder="" />
      </div>
      <x-slot name="footer">
        <div class="flex justify-end gap-x-2">
          <x-button flat label="Cancel" x-on:click="close" />
          <x-button wire:click="updateCharges" spinner="updateCharges" positive label="Update" />
        </div>
      </x-slot>
    </x-card>
  </x-modal> --}}
  ...
  <x-modal wire:model.defer="add_modal" align="center" max-width="lg">
    <x-card>
      <div class="header flex space-x-2 items-center">
        ...
        <h1 class="text-lg font-semibold uppercase text-gray-600 ">Add New Damages</h1>
      </div>
      ...
      <x-slot name="footer">
        <div class="flex justify-end gap-x-4">
          ...
          <x-button positive right-icon="save-as" wire:click="saveCharges" spinner="saveCharges" label="Save" />
        </div>
      </x-slot>
    </x-card>
  </x-modal>
