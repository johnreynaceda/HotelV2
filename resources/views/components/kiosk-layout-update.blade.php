  ...
  <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
    ...
    <div id="logoutModal" style="display: none;"
      ...
      <form method="POST" action="{{ route('logout') }}" class="flex space-x-2">
        @csrf
        <x-button type="button" id="cancelLogoutBtn" label="Cancel" sm icon="x" />
...
<script>
  ...
  //add javascript to handle the logout modal
document.addEventListener('DOMContentLoaded', function () {
  const logoutButton = document.querySelector('button[x-on\\:click="logout = true"]');
